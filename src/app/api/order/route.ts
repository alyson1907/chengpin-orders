...
const filterInsufficientStock = async (orderItems: { id: string; qty: number }[]) => {
  const currentAvailables = await findAvailables(orderItems)
  return orderItems.filter((b) => {
    const availableQty = currentAvailables.find((ca) => ca.id === b.id)?.qty || 0
    return b.qty > availableQty
  })
}

const buildOrderItemsInsert = async (orderItems: { id: string; qty: number }[]) => {
  const currentAvailables = await findAvailables(orderItems)
